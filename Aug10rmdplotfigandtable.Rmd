...
```{r}
head(dfrmdhmdacensus)
stargazer(dfrmdhmdacensus[c("Credit_Score","applicantincome","amountofloan","Original_Interest_Rate","diff_interest_mean","Original_DTI_Ratio","diff_interest_median","Original_Combined_LTV","Original_UPB","property_val","combined_loan_to_value_ratio","derived_race", "year")], type = "text",title="Descriptive statistics individual level Table 1",digits=1, out="individual leveltable1.txt")
...
df_population_penetration_all_years <- read.csv("df_final_income_population.csv")
 ...
 nrow(data_bbd)
 p <- ggplot(data= data_bbd, aes(x=year, y= broadband_penetration))+ geom_line()+ggtitle("Internet penetration in the U.S") +
  xlab("Year") + ylab("Internet Penetration Ratio")
plot(p)
```
head(df_population_penetration_all_years)
```{r}
...
q <- ggplot(data= data_bbd_filter_all_year, aes(x=year, y= ratio))+ geom_line() 
plot(q)
...
nrow(df_map_2019)
summary(df_map_union_2018_2019)
df_map_union_2018_2019[-6]
head(filter_map_20182019)
filter_map_20182019 <- sqldf("select * from df_map_union_2018_2019 where R19 is not NULL;")
chg <- filter_map_20182019$R19 - filter_map_20182019$R18
filter_map_20182019$chgper1819 <- (filter_map_20182019$R19 - filter_map_20182019$R18)*100/filter_map_20182019$R18

head(filter_map_20182019)
--------------------------2019-2020---------------------------------------
df_map_union_2019_2020 <- sqldf("select * from df_map_2019 a left join df_map_2020 b on a.fips=b.fips ;")
head(filter_map_2019_2020)
summary(df__union_2019_2020)
...
filter_map_2019_2020$chgper1920 <- (filter_map_2019_2020$R20 - filter_map_2019_2020$R19)*100/filter_map_2019_2020$R19
---------------------Map ---18--20----------------------------------
...
summary(df__union_2018_2020)
df_map_union_2018_2020 <- df_map_union_2018_2020[,-6]
filter_map_2018_2020 <- sqldf("select * from df_map_union_2018_2020 where R18 is not NULL;")
filter_map_2018_2020$chgper1820 <- (filter_map_2018_2020$R20 - filter_map_2018_2020$R18)*100/filter_map_2018_2020$R18
...
plot_usmap(data = df_18_20_change1000, values = "chgper1820") + scale_fill_continuous(
    low = "green", high = "magenta",
  name = "Change >= 50% Broadband Penetration (2018-2020)", label = scales::comma) + 
  theme(legend.position = "top")
  head(df_all_numeric)
  write.csv(df_all_numeric,"First_year_df_all_numeric.csv")
...
-------------------Regression Interest Rate ~ credit+x+y+z------------------------
  ...
  nrow(df_all_numeric)
  library(lfe)
  regress_tab4 <- felm(interest_rate ~ Credit_Score+Original_DTI_Ratio+applicantincome |countyid, data= df_all_numeric)
 stargazer(regress_tab4,title="Regression Results",single.row= TRUE, align=TRUE,type="text", out="regtab4.txt")
   -------------------------------BBD----------------------------------------------
   regress_tab6 <- felm(interest_rate~ ratio+Credit_Score+Original_DTI_Ratio+applicantincome |countyid, data= df_all_numeric)
   stargazer(regress_tab6,title="Regression Results",single.row= TRUE, align=TRUE,type="text", out="regtab6.txt")
   
   regress_tab8 <- felm(Original_UPB~ ratio+Credit_Score+Original_DTI_Ratio+applicantincome |countyid, data= df_all_numeric)
   stargazer(regress_tab8,title="Regression Results",single.row= TRUE, align=TRUE,type="text", out="regtab8.txt")
   
     regress_tab9 <- felm(interest_rate~ ratio |countyid, data= df_all_numeric)
   stargazer(regress_tab9,title="Regression Results",single.row= TRUE, align=TRUE,type="text", out="regtab9.txt")
...
write.csv(race_eth_data_1,"race and ethnicity data.csv")
**************************************************************************************8
*race_eth_data <- sqldf("SELECT * ,
                      CASE derived_race WHEN 'Asian' THEN 0
                      WHEN 'White' THEN 1
...
race_eth_data$ethnicity_factor <- as.factor(race_eth_data$ethnicity_factor)
 ...
 regress_tab11 <- felm(interest_rate~ ratio+Credit_Score+derived_race+derived_ethnicity|countyid, data= race_eth_data_1)
   stargazer(regress_tab11,title="Regression Results",single.row= TRUE, align=TRUE,type="text", out="regtab11.txt")
...
regress_tab23 <- felm(diff_int_yr_med~ ratio+Credit_Score+debt_to_income_ratio|year, data= df_all_diff_med)
       stargazer(regress_tab23,title=" diff_med_int ",single.row= TRUE, align=TRUE,type="text", out="incomeregtab23.txt")
